...
window.ASBS_FIGURES = {
  ...
  versions: [
    {id:"v22", title:"恢复 v20 视觉主线 · 对齐分段提议", scope:"segment", tag:"新候选 · 待作者视觉确认", date:"2026-09-24", available:["teaser","method","comparison"], sources:{teaser:"figures/history/v22-r2-teaser.png",method:"figures/history/v22-r2-method.png",comparison:"figures/history/v22-r2-comparison.png"}, changes:"保留 v20 的具体机械臂场景、衬线层次与粒子输运，删除 v21 的平铺文字卡片。区分续接评分、提议学习与分段执行；新版本原图、首稿和可编辑预览均归档。", limit:"新分段协议示意，运行效果未验证；不是已证明的机器人提升。生成原图与 PPT 重建分别保留。机械臂为概念插图；复杂轨迹与少数公式标签为可移动图片。", notes:{teaser:"将评分反馈保存为提议器：每段抽样一次，每个 chunk 重新观测并调用同一冻结 VLA。不是整个 segment 仅一次 VLA 前向。",method:"十个 chunk 的噪声流；点源 Z₀=0，原生高斯是无控制过程的终点。当前评分来源为模拟器，不把 Astra 扩展当成已完成实验。",comparison:"保留三栏监督机制对照；ASBS 是扩散采样器，不声称普遍优于 diffusion 或 flow。学习价值重排也能免在线 S2，必须公平比较。"}},
